# 🦞 OpenClaw Deployment — Личный AI-ассистент

Документация по развёрнутой инфраструктуре персонального AI-бота.

## 📋 Содержание

- [Архитектура](#архитектура)
- [Компоненты](#компоненты)
- [Эндпойнты и доступы](#эндпойнты-и-доступы)
- [Восстановление](#восстановление)
- [Безопасность](#безопасность)

---

## 🏗️ Архитектура

```
┌─────────────────────────────────────────────────────────────────┐
│                        ИНТЕРНЕТ                                  │
└─────────────────────────────┬───────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│  HETZNER VPS (46.224.221.0)                                     │
│  ├── OpenClaw Gateway (systemd)                                 │
│  ├── Telegram Bot (@neironassistant_bot)                        │
│  └── Claude Sonnet 4 (Anthropic API)                            │
└─────────────────────────────┬───────────────────────────────────┘
                              │ Tailscale VPN (encrypted)
                              │
┌─────────────────────────────┴───────────────────────────────────┐
│  MAC (nano-m4-macbook-pro)                                      │
│  ├── OpenClaw Node (LaunchAgent)                                │
│  ├── ~/Downloads (доступ для очистки)                           │
│  ├── ~/Desktop (доступ для очистки)                             │
│  └── Транзакционные бэкапы                                      │
└─────────────────────────────────────────────────────────────────┘
```

## 🔧 Компоненты

### 1. Hetzner VPS
- **IP:** `46.224.221.0`
- **Tailscale IP:** `100.73.176.127`
- **OS:** Ubuntu 24.04 LTS
- **План:** CPX22 (~€5/мес)
- **Пользователь:** `openclaw`

### 2. OpenClaw Gateway
- **Порт:** `18789`
- **Bind:** `lan` (0.0.0.0)
- **Auth:** token-based
- **Модель:** `anthropic/claude-sonnet-4-20250514`

### 3. Telegram Bot
- **Username:** `@neironassistant_bot`
- **DM Policy:** pairing
- **Approved User:** `685668909`

### 4. Mac Node
- **Name:** `mac-files`
- **Tailscale IP:** `100.91.12.108`
- **Service:** LaunchAgent (`ai.openclaw.node`)
- **Capabilities:** browser, system

### 5. Транзакционные бэкапы
- **Скрипты:** `~/.openclaw/cleanup-scripts/`
- **Бэкапы:** `~/.openclaw/cleanup-backups/`
- **Автоочистка:** cron (24 часа)

---

## 🔗 Эндпойнты и доступы

| Компонент | Адрес | Доступ |
|-----------|-------|--------|
| Gateway WS | `ws://100.73.176.127:18789` | Tailscale only |
| Control UI | `http://100.73.176.127:18789/` | Tailscale only |
| SSH Server | `ssh openclaw@46.224.221.0` | SSH key |
| SSH Tailscale | `ssh openclaw@100.73.176.127` | SSH key |
| Telegram | `@neironassistant_bot` | Pairing code |

---

## 🔐 Токены и ключи

> ⚠️ **ВАЖНО:** Храни секреты в безопасном месте, не в репозитории!

| Секрет | Расположение | Описание |
|--------|--------------|----------|
| Gateway Token | `~/.openclaw/openclaw.json` (сервер) | Аутентификация WS |
| Anthropic API Key | systemd env (сервер) | Claude API |
| Telegram Bot Token | `~/.openclaw/openclaw.json` (сервер) | Bot API |
| SSH Key | `~/.ssh/id_ed25519` (Mac) | Доступ к серверу |

---

## 🔄 Восстановление

### Команды через Telegram (самый простой способ)

| Команда | Действие |
|---------|----------|
| `/restart` | Перезапустить gateway |
| `/clear` | Очистить сессию (при переполнении) |
| `/compact` | Сжать контекст (если тормозит) |
| `/status` | Проверить состояние |

### Автоматическая защита (настроено)
- **Watchdog**: перезапуск если процесс не отвечает 60 сек
- **Memory limit**: перезапуск если память > 1GB
- **Context pruning**: автоочистка старых tool results

### Через SSH (если Telegram не работает)

```bash
# Перезапуск Gateway
ssh openclaw@100.73.176.127 "systemctl --user restart openclaw-gateway"

# Очистка сессии + перезапуск
ssh openclaw@100.73.176.127 "rm -f ~/.openclaw/agents/main/sessions/sessions.json && systemctl --user restart openclaw-gateway"

# Проверить статус
ssh openclaw@100.73.176.127 "export PATH=/home/openclaw/.npm-global/bin:\$PATH && openclaw status"
```

### Перезапуск Mac Node

```bash
openclaw node restart
```

### Полное восстановление

См. файл: [RECOVERY.md](./RECOVERY.md)

---

## 🛡️ Безопасность

### Реализованные меры:
- ✅ UFW firewall (только SSH открыт)
- ✅ Fail2ban (защита от brute-force)
- ✅ SSH key-only (пароли отключены)
- ✅ Non-root пользователь
- ✅ Tailscale VPN (шифрование)
- ✅ Gateway token auth
- ✅ Telegram pairing (код подтверждения)
- ✅ Транзакционные бэкапы
- ✅ Allowlist команд на node

### Правила работы с файлами:
- Каждое удаление требует подтверждения
- Бэкап создаётся перед сессией очистки
- Откат возможен до подтверждения
- Бэкапы автоудаляются через 24 часа

---

## 📁 Связанные файлы

- [RECOVERY.md](./RECOVERY.md) — Инструкции по восстановлению
- [server-config.md](./server-config.md) — Конфигурация сервера
- [mac-config.md](./mac-config.md) — Конфигурация Mac
- [cleanup-scripts.md](./cleanup-scripts.md) — Скрипты очистки

---

*Последнее обновление: 2026-02-05*
